...
class InfGraph:public Graph
{
    public:
		...
		double BuildSeedSet()
		{
            ...
            set<int> seedSetConverted;
            for(int s: seedSet) {
                seedSetConverted.insert(s);
            }
            vector<int> activatedSet = performDiffusion(seedSetConverted);
    ...
    void singleDiffusion(set<int> *activatedSet, set<int> *seedSet, vector<int> *visitMark, vector<bool> *visited, deque<int> *queue) {
        ...
        for(int seed: *seedSet) {
            ...
            while(!queue->empty()) {
                ...
                for (int v : originalGraph[u]) {
                    int randomNumber = rand() % inDeg[v];
                    bool activeEdge = (randomNumber==0);
                    if (activeEdge) {
                        if(!(*visited)[v])
    ...
    vector<int> performDiffusion(set<int> seedSet) {
        int activatedFrequency[this->n];
        ...
        int NUMBER_OF_SIMULATIONS = 20000;
        for (int i=0; i<NUMBER_OF_SIMULATIONS; i++) {
            singleDiffusion(activatedSet, &seedSet, &visitMark, &visited, diffusionQueue);
        ...
        priority_queue<pair<int, int>, vector<pair<int, int>>, CompareBySecond> queue;
